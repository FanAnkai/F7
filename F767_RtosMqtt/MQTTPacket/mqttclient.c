...
uint8_t payload[] = "{\"id\"\:1610289611118\,\"Params\"\:{\"LightSwitch\"\:1\,\"LightLuminance\"\:88\}\,\"ResultData\"\:\{\}\,\"Reason\"\:\"success\"\}";
    ...
    while(1)
    {
        ...
        if(xReturn > 0)
        {
            ...
            if(UPDATE_SUCCESS == res)
            {
                ...
                ret = MQTTMsgPublish(mqtt_socket, (char*)TOPIC3, QOS0, (uint8_t*)q);
                if(ret >= 0)
                {
                    //表明有数据交换
                    no_mqtt_msg_exchange = 0;
