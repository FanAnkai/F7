...
static char Encode_GetChar(BYTE num);
...
int iot_decode_base64(char *pDest, type_size_t dstlen, const char *pSrc, type_size_t srclen)
{
	BYTE input[4] = {0};
	type_size_t i, index = 0;
	type_size_t c = 0;

	for (i = srclen - 1; i > srclen - 3; i--) {
		if (pSrc[i] == '=')
			c++;
	}

	if (dstlen < (srclen * 3 / 4 - c)) {
		return -1;
	}
...
int iot_encode_base64(char *pDest, type_size_t dstlen, const char *pSrc, type_size_t srclen)
{
	BYTE input[3], output[4];
	type_size_t i, index_src = 0, index_dest = 0;
